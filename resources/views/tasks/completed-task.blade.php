<?php $sn = ($completedTasks->perPage() * ($completedTasks->currentPage()-1)); ?>

@extends('layouts.layouts')
@section('title', 'Ultrabyte | Completed Tasks')
	...
	<section class="content">
		...
		<div class="box">
			...
			<div class="box-body table-responsive no-padding">
					...
					@if(!$completedTasks->isEmpty())
						@foreach( $completedTasks as $completeTask )
							...
							<tr>
								<td>{{ $sn }}</td>
								<td>
									<a href="#" class="companyDetail" data-companyid="{{ $completeTask->company_id }}">{{ $completeTask->company->name }}</a>
								</td>
								<td> {{ $completeTask->service->name }}</td>
								
								<td><?php 
									$endDate = new DateTime($completeTask->expiration_date);
		        					$days = $currentDate->diff($endDate)->days;
		        					$updatedExpirationDate = new DateTime($completeTask->updated_expiration_date);
		        					
		        					if( $completeTask->updated_expiration_date != NULL )
		        					{
								...
								<td>
									<a href="" class="btn btn-danger btn-xs" title="Mark as task complete" data-completedtaskid="{{ $completeTask->id }}" data-toggle="modal" data-target="#markAsComplete">Mark as task due</a>
								</td>
							</tr>
						@endforeach
					@endif
				...
				@if( $completedTasks->total() > $completedTasks->perPage() )
					{{ $completedTasks->links('layouts.paginator') }}
		        @else
	...
	<div class="modal fade" id="markAsComplete" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				...
				<form method="POST" action="{{ url('/unmarked') }}"> 
					{{ csrf_field() }}
...
@section('script')
	<script>
		$('#markAsComplete').on('show.bs.modal', function (event) {
			...
			var taskid = button.data('completedtaskid');
			var modal = $(this);
