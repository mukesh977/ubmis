...
@section('title', 'Ultrabyte | Status')
			...
			<div class="box box-primary">
				<div class="box-header with-border">
					...
					{{-- <div class="dropdown pull-right">
						<a href="{{ url('/admin/status/create') }}" class="btn btn-primary">Create Status</a>
					</div> --}}
				...
				<div class="box-body table-responsive no-padding">
					...
					<table class="table table-hover">
						<tbody>
							...
							<tr>
								<td>{{ $key+1 }}</td>
								<td style="color: {{$stat->color}}">{{ $stat->name }}</td>
								<td>
									<form action="{{ url('admin/status/'.$stat->id) }}" method="post" class="form-inline">
									@csrf
									@method('delete')
									<a href="{{ url('admin/status/'. $stat->id .'/edit') }}" class="btn btn-primary"><i class="fa fa-edit"></i></a> <button class="btn btn-danger" role="submit" onclick="return confirm('Are you sure?')"> <i class="fa fa-trash"></i> </button>
								</form>
								</td>
							</tr>
							@endforeach
