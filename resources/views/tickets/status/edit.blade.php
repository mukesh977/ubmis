...
@section('title', 'Ultrabyte | Edit Status')
		...
		<div class="col-md-12">
			<div class="box box-primary">
				<div class="box-header with-border">
					...
					<h3 class="box-title">Edit {{ $status->name }}'s Status</h3>
				</div>
				<div class="box-body table-responsive no-padding">
					<form method="post" action="{{ url('admin/status/'.$status->id) }}" method="post">
						@csrf
						@method('put');
						<div class="box-body">
							<div class="form-group">
								<label for="exampleInputPassword1">Status Name</label>
								<input type="text" class="form-control" id="exampleInputPassword1" name="name" value="{{ (old('name'))? old('name'): $status->name }}">
								@if($errors->has('name'))
							...
							<div class="form-group">
								...
								<input type="color" name="color" value="{{ (old('color')? old('color'): $status->color) }}" class="form-control">
								<span class="form-text text- ">If you select red color  then status name will be displayed in red color.</span>
							</div>
						</div>
						<!-- /.box-body -->
