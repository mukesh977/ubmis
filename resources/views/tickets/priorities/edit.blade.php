...
@section('title', 'Ultrabyte | Edit Priorities')
		...
		<div class="col-md-12">
			<div class="box box-primary">
				<div class="box-header with-border">
					...
					<h3 class="box-title">Edit {{ $priorities->name }}'s Priority</h3>
				</div>
				<div class="box-body table-responsive no-padding">
					<form method="post" action="{{ url('admin/priorities/'.$priorities->id) }}" method="post">
						@csrf
						@method('put');
						<div class="box-body">
							<div class="form-group">
								<label for="exampleInputPassword1">priorities Name</label>
								<input type="text" class="form-control" id="exampleInputPassword1" name="name" value="{{ (old('name'))? old('name'): $priorities->name }}">
								@if($errors->has('name'))
							...
							<div class="form-group">
								...
								<input type="color" name="color" value="{{ (old('color')? old('color'): $priorities->color) }}" class="form-control">
								<span class="form-text text- ">If you select red color  then priorities name will be displayed in red color.</span>
							</div>
						</div>
						<!-- /.box-body -->
