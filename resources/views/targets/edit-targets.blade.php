            ...
            <div class="col-xs-12 col-md-12">
                <div class="box box-success">
                    <div class="box-header with-border">
                        <h3 class="box-title">{{ trans('Target Edit') }}</h3>
                    ...
                    <div class="box-body">
                        <form role="form" id="target-form" method="POST" action="{{route('users-targets.update', $userTargets->id)}}">
                            {{csrf_field()}}
                            {{ method_field('PUT') }}
                            <div class="box-body">
                                <div class="form-group {{ $errors->has('user_id') ? ' has-error' : '' }} ">
                                    ...
                                    <select name="user_id" id="user_id" class="form-control">
                                        ...
                                        @forelse($users as $user)
                                            @if(!empty($userTargets->users))
                                                @if($userTargets->users->id == $user->id)
                                ...
                                <div class="form-group {{ $errors->has('target_id') ? ' has-error' : '' }}">
                                    ...
                                    <select name="target_id" id="target_id" class="form-control">
                                        ...
                                        @forelse($targets as $target)
                                            @if(!empty($userTargets->targets))
                                                @if($userTargets->targets->id == $target->id)
				                                    <?php $selected = 'selected'; ?>
                                                @else
				                                    <?php $selected = ''; ?>
                                                @endif
                                ...
                                <div class="form-group {{ $errors->has('total_target') ? ' has-error' : '' }}">
                                    ...
                                    <input type="text" name="total_target" id="total_target" class="form-control" placeholder="e.g. 100" value="{{ $userTargets->total_target }}">
