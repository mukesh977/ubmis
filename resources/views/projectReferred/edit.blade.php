...
@section('title', 'Ultrabyte | Edit Project Referred Relationship')
				...
				<div class="box box-primary">
					<div class="box-header with-border">
						...
						<div class="box-tools pull-right">
				            <a href="{{ url('/admin/referred') }}" class="btn btn-primary btn-xs">Edit Project Referred Relationship</a>
				        </div>

					</div>

					<div class="box-body">
						<form method="post" action="{{ url('/admin/referred/'. $sT->id.'/'. $p->id) }}">
							{{ csrf_field() }}
									...
									<div class="form-group" id="form-group-name">
							          	...
							          	<select class="form-control select2" name="salesCode">
								          	...
								          	@foreach( $salesTransactions as $salesTransaction )
								          		<option value="{{ $salesTransaction->id }}" {{ (($sT->id)==($salesTransaction->id))? 'selected' : '' }}>
							        ...
							        <div class="form-group" id="form-group-address">
							          ...
							          <select class="form-control select2" name="personName">
								          	...
								          	@foreach( $personNames as $personName )
								          		<option value="{{ $personName->id }}" {{ ($p->id==($personName->id))? 'selected' : '' }}>
							                    ...
							                    <td width="95%">
							                      	<select class="form-control select2" name="parentName[]">
										          		...
										          		@if( !$editParent->isEmpty())
											          		@foreach( $editParent as $eP )
													          	@foreach( $personNames as $personName )
													          		<option value="{{ $personName->id }}" {{ (($eP->parent_name_id)==$personName->id)? 'selected' : '' }}>
													          			{{ $personName->first_name .' '. $personName->last_name }}
													          		</option>
													          	@endforeach
												          	@endforeach
							    					...
							    					<tr>
							    						<td width="95%">
							    							<select class="form-control select2" name="childName[]">
												          		...
												          		@if( !$editChild->isEmpty())
													          		@foreach( $editChild as $eC )
															          	@foreach( $personNames as $personName )
															          		<option value="{{ $personName->id }}" {{ ($eC->child_name_id==$personName->id)? 'selected' : '' }}>
															          			{{ $personName->first_name .' '. $personName->last_name }}
															          		</option>
															          	@endforeach
														          	@endforeach
