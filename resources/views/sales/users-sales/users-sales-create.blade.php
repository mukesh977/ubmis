            ...
            <div class="col-xs-12 col-md-12">
                <div class="box box-success">
                    <div class="box-header with-border">
                        <h3 class="box-title">{{ trans('Sales Create') }}</h3>
                        ...
                        <div class="box-body">
                            <div class="form-group {{ $errors->has('date') ? ' has-error' : '' }}">
                                ...
                                <div class="input-group date">
                                    ...
                                    <input type="text" class="form-control pull-right" name="date" id="date" value=" @if(old('date')){{ old('date') }} @else {{ \Carbon\Carbon::now()->format('m/d/Y') }} @endif">
                                </div>
                                @if ($errors->has('date'))
                            ...
                            <div class="form-group {{ $errors->has('company_id') ? ' has-error' : '' }}" id="office">
                                ...
                                <select name="company_id" id="company_id" class="form-control">
                                    ...
                                    <option value="{{ $data->id }}" {{ ( old('company_id') == $data->id ? "selected" : '') }}>{{ $data->name }}</option>
                                    @empty
                                    @endforelse
                                </select>
                            ...
                            <div class="form-group {{ $errors->has('sales_category_id') ? ' has-error' : '' }}">
                                ...
                                <select name="sales_category_id" id="sales_category_id" class="form-control">
                                    ...
                                    @forelse($salesCategories as $data)
                                        <option value="{{ $data->id }}" {{ ( old('sales_category_id') == $data->id ? "selected" : '') }}>{{ $data->name }}</option>
                                    @empty
                                    @endforelse
                                </select>
                            ...
                            <div class="form-group {{ $errors->has('received_amount') ? ' has-error' : '' }}">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="text" name="received_amount" id="received_amount" class="form-control" placeholder="e.g. 100000" value="{{ old('received_amount') }}">
